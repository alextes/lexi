...
async fn run_proactive_slot_check<D: Db + Clone + Send + Sync + 'static>(
    ctx: BotContext<D>,
    ...
    .await
    {
        ...
        info!(chat_id = telegram_chat_id, "sent proactive slot check reply");
    }
}

pub fn maybe_schedule_proactive_slot_check<D: Db + Clone + Send + Sync + 'static>(
    ctx: &BotContext<D>,
    telegram_chat_id: i64,
    message_text: &str,
) {
    ...
    tokio::spawn(run_proactive_slot_check(ctx_clone, telegram_chat_id, full_message));
}
